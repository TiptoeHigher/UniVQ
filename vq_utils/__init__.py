...
def zero_pad_high_freq(xf, copy=False):
    ...
    """
    if not copy:
        # 方法1：使用scatter替代zeros初始化
        xf_l = torch.zeros_like(xf)  # 使用zeros_like替代zeros+to(device)
        xf_l[:, :, 0, :] = xf[:, :, 0, :]
        return xf_l
    else:
        # 方法2：使用expand替代repeat
        xf_l = xf[:, :, [0], :]  # (b c 1 w)
        return xf_l.expand(-1, -1, xf.shape[2], -1).contiguous()  # (b c h w)
